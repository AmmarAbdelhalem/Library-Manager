...
def books_all() -> List[Tuple]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, title, author, year, category, description FROM books ORDER BY title")
    rows = cur.fetchall()
    conn.close()
    return rows
...
def borrow_book(user_id: int, book_id: int, days: int = 14) -> int:
    ...
    cur.execute("INSERT INTO borrows (user_id, book_id, due_date) VALUES (?, ?, datetime('now', ?))", (user_id, book_id, modifier))
    conn.commit()
    bid = cur.lastrowid
    conn.close()
    return bid
...
def return_book(user_id: int, book_id: int) -> bool:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id FROM borrows WHERE user_id = ? AND book_id = ? AND returned_at IS NULL ORDER BY borrowed_at DESC",
                (user_id, book_id))
    row = cur.fetchone()
    if not row:
        conn.close()
        return False
    borrow_id = row[0]
    cur.execute("UPDATE borrows SET returned_at = CURRENT_TIMESTAMP WHERE id = ?", (borrow_id,))
    conn.commit()
    conn.close()
    return True
...
def favorites_add(user_id: int, book_id: int) -> bool:
    ...
    cur = conn.cursor()
    try:
        cur.execute("INSERT OR IGNORE INTO favorites (user_id, book_id) VALUES (?, ?)", (user_id, book_id))
        conn.commit()
        ok = cur.rowcount > 0
    except Exception:
        ok = False
...
def favorites_remove(user_id: int, book_id: int) -> bool:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("DELETE FROM favorites WHERE user_id = ? AND book_id = ?", (user_id, book_id))
...
def favorites_by_user(user_id: int) -> List[Tuple]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT b.id, b.title, b.author, b.year, b.category, f.added_at FROM favorites f JOIN books b ON f.book_id = b.id WHERE f.user_id = ? ORDER BY f.added_at DESC", (user_id,))
    rows = cur.fetchall()
    conn.close()
    return rows
