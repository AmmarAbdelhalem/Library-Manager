...
class AuthPage(tk.Frame):
    def __init__(self, parent, on_success):
        ...
        tk.Label(self, text="Welcome — please sign in", font=("Arial", 16)).pack(
            pady=(10, 8)
        )
        ...
        tk.Entry(frm, textvariable=self.password, show="•").grid(
        ...
        ttk.Button(btn_frame, text="Sign in", command=self.try_signin).grid(
            row=0, column=0, padx=6
        )
        ttk.Button(btn_frame, text="Register", command=self.try_register).grid(
    ...
    def try_register(self):
        ...
        if not uname or not pwd:
            messagebox.showwarning(
                "Missing", "Please enter username and password to register."
            )
            return
